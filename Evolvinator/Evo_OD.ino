...
float diodeReading;                         // raw diode reading [0-1023]                      
...
float ODRead() {
  ...
  float diodeLightOnAvg = 0;
  float diodeLightOffAvg = 0;
  ...
  for (int y = 0; y < 5; y++) {                   // average over 5 readings
    ...
    diodeLightOffAvg = photoRead / 30.0;            // average
    digitalWrite(pinODLED, HIGH);                 // turn light on
    delayMicroseconds(1);                         // give the photodiode a microsecond to adjust
    photoRead = 0;
    for (int x = 0; x < 30; x++) {
      photoRead += analogRead(pinODRead);
      delay(10);
    }
    diodeLightOnAvg = photoRead / 30.0;
  ...
  ODMin[0] = (log(1 / (ODMin[0] / ODZero)) / log(10));  //gives NaN until calibrated
...
float ODCalibrate() {
  ...
  for (int y = 0; y < 10; y++) {
    photoRead = 0; 
    for (int x = 0; x < 30; x++) {             //take 30 readings
      photoRead += analogRead(pinODRead);
      delay(10);
    }
    diodeLightOffAvg = photoRead / 30;        // average readings with LED off
    digitalWrite(pinODLED, HIGH);             // turns LED on (HIGH)       
    delayMicroseconds(1);                            
    photoRead = 0;
    for (int x = 0; x < 30; x++) {            //take 30 readings
      photoRead += analogRead(pinODRead);
    ...
    diodeLightOnAvg = photoRead / 30;         // average readings with LED on
    ODZero += float(diodeLightOnAvg - diodeLightOffAvg);     // ODZero = light on - light off 
    digitalWrite(pinODLED, LOW);                // turns LED off
    delayMicroseconds(1);
  }
  ODZero /= 10;                                 // Avg ODZero readings
  ...
  if (debugMode) {
    Serial.print("Averaged ODZero is ");
